...
public class DBQueries_RFL {
	...
	public static String GET_RANDOM_INACTIVE_CONSULTANT_EMAILID = 
			"SELECT TOP 1 "+
					"ac.AccountID,ac.AccountNumber[CID],REPLACE(EmailAddress,'terminated_','')[Original_UserEmail],EmailAddress "+
					"FROM    RodanFieldsLive.dbo.Accounts (NOLOCK) ac "+
					"WHERE   ac.AccountTypeID = 1 "+
					" AND ac.MarketID = 1 "+
					"AND ac.Active = 0 "+
					"AND ac.EmailAddress  LIKE 'terminated_%@rodanandfields.com' "+
					"ORDER BY NEWID()"; 
	...
	public static String GET_RANDOM_ACTIVE_CONSULTANT_ORDER_DETAILS_RFO="select top 1 O.OrderNumber, O.SubTotal, O.TotalTax, OS.ShippingCost,"+
			" O.Total, O.QV, O.CV, O.CompletionDate from Hybris.Orders as O"+
			" join Hybris.OrderPayment as OP ON OP.OrderID = O.OrderID"+
			" join Hybris.OrderShipment as OS ON OS.OrderID = O.OrderID"+
			" where O.AccountID in"+
			"  (Select top 1 AB.AccountID from RFO_Accounts.AccountBase as AB"+
			"  join RFO_Accounts.AccountRF as ARF ON ARF.AccountID = AB.AccountID"+
			"    where AB.AccountTypeID = 1 and AB.CountryID = 236 "+
			"    and AB.AccountStatusID = 1 and ARF.EnrollmentDate > DATEADD(Day, -180, GETDATE()) order by newid()) "+
	...
	public static String GET_RANDOM_ACTIVE_USERS_WITH_PENDING_ORDERS_RFL = 
			"SELECT TOP 1 "+
					"ac.AccountNumber[CID] , "+
					"ac.EmailAddress  "+
					...
					"AND ac.AccountID<>2 "+
					"AND EXISTS ( SELECT 1 "+
					"FROM   RodanFieldsLive.dbo.OrderCustomers oc "+
					"JOIN RodanFieldsLive.dbo.Orders o ON o.OrderID = oc.OrderID "+
					"WHERE  oc.AccountID = ac.AccountID "+
	...
	public static String GET_RANDOM_ACTIVE_PC_WITH_ORDERS_AND_AUTOSHIPS_RFL = "SELECT TOP 1 a.EmailAddress as UserName FROM dbo.Accounts As a WHERE   a.AccountTypeID = 2 and a.Active = 1 ORDER BY NEWID()";
	...
	public static String GET_RANDOM_ACTIVE_RC_HAVING_ORDERS =
			...
			"AND [as].UserName  LIKE '%@rodanandfields.com' "+
			/*Active Account*/ 
			"AND EXISTS ( SELECT 1 "+
			"FROM   dbo.Accounts AS a3 "+
			"WHERE  a3.Active = 1 "+
	...
	public static String GET_ACTIVE_CONSULTANT_WHOSE_CRP_IS_INACTIVE=
			"SELECT TOP 1 "+
					"ab.AccountID , "+
					"ab.AccountNumber [AccountNumber] , "+
					"acs.Username [EmailAddress] "+
					...
					"FROM   RFOperations.Hybris.Autoship aa "+
					"WHERE  aa.AccountID = ab.AccountID "+
					"AND aa.AutoshipTypeID = 2 "+
					"AND aa.Active = 1 ) "+
					"ORDER BY NEWID()";
...
"ORDER BY NEWID()";
	...
	public static String GET_RANDOM_ACTIVE_CONSULTANT_PREFIX="select top 1 dbo.SiteURLs.URL from dbo.SiteURLs order by newid()";
	public static String GET_ACCOUNT_STATUS_ID_OF_INACTIVE_PC = "select * from dbo.Accounts where Active=0 and emailAddress = '%s'";
	public static String GET_RANDOM_ACTIVE_PC_WITH_ORDERS_AND_AUTOSHIPS_DETAILS_RFL = "SELECT TOP 1 * FROM dbo.Accounts As a WHERE   a.AccountTypeID = 2 and a.Active = 1 ORDER BY NEWID()";	
	public static String GET_RANDOM_ACTIVE_CONSULTANT_FOR_PULSE_CANCELLATION_FROM_SITE_SUBSCRIPTION=
			"SELECT TOP 1 "+
					"ab.AccountID , "+
					"ab.AccountNumber [AccountNumber] , "+
					"acs.Username [EmailAddress] "+
	...
	public static String GET_RANDOM_INACTIVE_ACCOUNT_DETAILS = "select top 1 * from Accounts where Active!='1' ORDER BY NEWID()";
	public static String GET_RANDOM_ACTIVE_CONSULTANT_WITH_INACTIVE_CRP_AND_PULSE_TEMPLATE_RFL = 
			"USE RodanFieldsLive "+
					"SET TRANSACTION  ISOLATION LEVEL READ UNCOMMITTED; "+
					"BEGIN TRANSACTION "+
					"SELECT TOP 1 "+
					...
					"WHERE  oc.AccountID = a.AccountID "+
					"AND o.OrderTypeID = 5 /*Consultant Auto-ship Template*/ "+
					"AND ao.AutoshipScheduleID in (1,3) /*Consultant Replenishment and Pulse Template*/"+
					"AND o.OrderStatusID = 7) /*Cancelled Template*/"+
					"ORDER BY NEWID()";	

	public static String GET_ORDER_DETAILS_BY_DATE=
			"SELECT top 1 dbo.Accounts.FirstName,dbo.Accounts.LastName, dbo.Orders.CompleteDate, dbo.Orders.OrderNumber "+
					"FROM dbo.Orders "+ 
					"JOIN dbo.OrderCustomers ON dbo.OrderCustomers.OrderID = dbo.Orders.OrderID "+ 
					"JOIN dbo.OrderShipments ON dbo.OrderShipments.OrderID = dbo.Orders.OrderID "+ 
					"JOIN dbo.OrderPayments ON dbo.OrderPayments.OrderID = dbo.Orders.OrderID "+ 
					"JOIN dbo.Accounts ON dbo.Accounts.AccountID = dbo.OrderCustomers.AccountID "+ 
					"WHERE dbo.Orders.CompleteDate > DATEADD(Day, -180, GETDATE()) "+
					"ORDER BY NEWID()";
	
	public static String GET_RANDOM_PC_HAS_BILLING_PROFILE_WITH_INVALID_CARD =
			"DECLARE @Today DATE = GETDATE()"+ 
					...
					"INNER JOIN dbo.AccountPaymentMethods apm ON apm.AccountID = a1.AccountID "+  
					"INNER JOIN dbo.AccountSecurity [as] ON [as].AccountID = a1.AccountID "+ 
					"WHERE a1.Active = 1 "+ 
					"AND a1.AccountTypeID = 2 "+ 
					"AND NOT EXISTS ( "+ 
					"SELECT TOP 1 o.OrderID "+ 
					"FROM dbo.Orders o "+ 
					"JOIN dbo.OrderCustomers oc ON oc.OrderID = o.OrderID "+ 
					"JOIN dbo.Accounts a ON a.AccountID = oc.AccountID "+ 
					"JOIN dbo.AutoshipOrders ao ON o.ParentOrderID = ao.TemplateOrderID "+ 
					"WHERE  oc.AccountID = a1.AccountID "+ 
					"AND ao.AutoshipScheduleID = 2 "+ 
					"AND a.MarketID = 1 "+ 
					"AND DATEPART(MONTH, o.StartDate) = DATEPART(MONTH, @Today) "+ 
					"AND DATEPART(YEAR, o.StartDate) = DATEPART(YEAR, @Today) "+ 
					"AND o.OrderStatusID = 4 "+ 
					"AND o.OrderTypeID =  CASE a.AccountTypeID "+ 
					"WHEN 1 THEN 7  "+ 
					"WHEN 2 THEN 6  END) "+ 
					"AND (SELECT a2.AccountTypeID FROM dbo.Accounts a2 WHERE a2.AccountID = a1.SponsorID) = 1 "+ 
					"AND apm.ExpDate < GETDATE()"+
					"ORDER BY newID()";
