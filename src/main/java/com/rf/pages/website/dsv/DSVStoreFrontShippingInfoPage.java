...
public class DSVStoreFrontShippingInfoPage extends DSVRFWebsiteBasePage {
	...
	private static final By VERIFY_YOUR_SHIPPING_ADDRESS_POPUP = By.xpath("//*[contains(@id,'AcceptOriginal')]");
	...
	public void enterNewShippingAddressDetails(String name,String addressLine1,String city,String state,String postalCode,String phoneNumber,String securityCode) {
		...
		provinceDD.selectByVisibleText(state);
	...
	public void clickSaveAddressBtn(){
		...
		driver.waitForLoadingImageToDisappear();
		if(driver.quickWaitForElementPresent(VERIFY_YOUR_SHIPPING_ADDRESS_POPUP)==true){
	...
	public void enterEditedShippingProfileName(String shippingName){
		logger.info("Shipping name entered for edit is "+shippingName);
		driver.waitForElementPresent(NAME_TXT_FIELD);
		driver.type(NAME_TXT_FIELD, shippingName);
	}

	public boolean areMoreShippingProfilesLeftForDeletion(){
		boolean areMoreShippingProfilesLeftForDeletion=false;
		if(driver.findElements(By.xpath("//div[@id='multiple-billing-profiles']/descendant::a[contains(text(),'Delete')][1]")).size()>0){
			areMoreShippingProfilesLeftForDeletion=true;
		}

		else{
			areMoreShippingProfilesLeftForDeletion=false;
		}
		return areMoreShippingProfilesLeftForDeletion;

	}

	public int getTotalShippingProfiles(){
		List<WebElement> allBillingProfiles =  driver.findElements(By.xpath("//div[@id='multiple-billing-profiles']//a[text()='Delete']"));
