...
public class CSCockpitAutoshipTemplateTabPage extends CSCockpitRFWebsiteBasePage{
	...
	private static final By ALL_PRODUCT_NAME_IN_AUTOSHIP_CART = By.xpath("//convertCRPDateToFormat[contains(@class,'csListItem')]//div[contains(@class,'CellLine1')][1]//span");
	...
	public String convertCRPDateToFormat(String UIDate){
		...
		if(driver.getCountry().equalsIgnoreCase("au")) {
			...
			logger.info("Next due date in format "+UIMonth+" "+date+", 20"+year);
	...
	public boolean verifyThresholdPopup(){
		boolean isPresent=false;
		if(driver.getCountry().equalsIgnoreCase("us")){
			driver.quickWaitForElementPresent(THRESHOLD_POPUP_FOR_US);
			isPresent= driver.isElementPresent(THRESHOLD_POPUP_FOR_US);
		}else {
			driver.quickWaitForElementPresent(THRESHOLD_POPUP_FOR_CA);
			isPresent= driver.isElementPresent(THRESHOLD_POPUP_FOR_CA);
		}
		return isPresent;
	}
	...
	public String getShippingAddressLine1UnderShippingAddress(){
		driver.waitForElementPresent(SHIPPING_ADDRESS_LINE1,50);
		return driver.findElement(SHIPPING_ADDRESS_LINE1).getText().trim();
	}

	public String getProductNumberFromSKUAtAutoship(String SKU){
		List<WebElement> productSKU = driver.findElements(ALL_PRODUCT_NAME_IN_AUTOSHIP_CART);
		int count =1;
	...
	public void clickPaymentInfoAddressDropDown(){
		driver.waitForElementToBeInVisible(PAYMENT_INFO_ADDRESS_DD,20);
		driver.click(PAYMENT_INFO_ADDRESS_DD,"Payment Info Address DropDown");
		driver.pauseExecutionFor(2000);
		driver.waitForCSCockpitLoadingImageToDisappear();
	}
