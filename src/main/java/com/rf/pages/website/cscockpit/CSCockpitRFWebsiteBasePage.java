...
public class CSCockpitRFWebsiteBasePage extends RFBasePage{
	...
	private static final By BILLING_ADDRESS_VALUE_ON_ADD_NEW_BILLING_PROFILE_POPUP = By.xpath("//span[text()='Select Billing Address']/following::td[@class='z-combo-item-text' and not(contains(text(),'CRM'))][1]"); 
	private static final By CREDIT_CARD_VERIFICATION_FAILED_ERROR_LOC=By.xpath("//*[contains(text(),'Credit card validation failed.')]");
	private static final By SECOND_BILLING_ADDRESS_VALUE_ON_ADD_NEW_BILLING_PROFILE_POPUP = By.xpath("//div[contains(@class,'csAddCardPaymentWidgetFrame')]//span[text()='Select Billing Address']/following::td[@class='z-combo-item-text'][2]");
	private static final By ADDRESS_LINE_TEXT_BOX = By.xpath("//span[text()='Line 1' or text()='Street Address']/following::input[1]"); 
	...
	private static final By CITY_TOWN_TEXT_BOX_LOC = By.xpath("//span[text()='Town' or text()='Suburb/Town']/following::input[1]");
	private static final By POSTAL_TEXT_BOX = By.xpath("//span[text()='Postal Code' or text()='Postcode']/following::input[1]");
	private static final By PROVINCE_TEXT_BOX = By.xpath("//span[text()='State/Province' or text()='State/Territory']/following::input[1]");
	private static String selectOptionFromAutoCompleteList = "//div[contains(@class,'autoCompletePopup')]//span[text()='%s']";
	private static String prductNameWithSKU =  "//span[contains(text(),'%s')]";
	private static final By ORDER_TAB = By.xpath("//span[text()='Order']");
	private static final By CANCEL_AUTOSHIP_BTN = By.xpath("//a[contains(text(),'Cancel Autoship')]");
	...
	public String getPSTDate(){
		...
		if(driver.getCountry().equalsIgnoreCase("au")) {
			pstTimeZone = TimeZone.getTimeZone("Australia/Sydney");

		} else {
			pstTimeZone = TimeZone.getTimeZone("America/Los_Angeles");
		}
	...
	public void clickCustomerTab(){
		...
		driver.clickByJS(RFWebsiteDriver.driver, CUSTOMER_TAB,"");
		driver.waitForCSCockpitLoadingImageToDisappear();
	}

	public void clickOrderSearchTab(){
		driver.waitForCSCockpitLoadingImageToDisappear();
		driver.click(ORDER_SEARCH_TAB_BTN,"Order search tab");
		driver.waitForCSCockpitLoadingImageToDisappear();
	}
	...
	public void selectValueFromSortByDDInCartTab(String valueFromDropDown){
		driver.click(By.xpath(String.format(sortByDropDownLoc, valueFromDropDown)),"Sort value as "+valueFromDropDown);
		driver.waitForCSCockpitLoadingImageToDisappear();
	}
	...
	public void searchSKUValueInCartTab(String SKUValue){
		...
		driver.type(SEARCH_SKU_VALUE_TXT_FIELD, SKUValue,"sku value");
		driver.click(SEARCH_BTN_SKU,"search SKU button");
		driver.waitForCSCockpitLoadingImageToDisappear();
	}
	...
	public void clickUseEnteredAddressbtnInEditAddressPopup(){
		...
		driver.waitForElementToBeInVisible(USE_ENTERED_ADDRESS_DISABLED_BTN_IN_QAS_POPUP, 5);
		driver.waitForPageLoad();
		driver.waitForCSCockpitLoadingImageToDisappear();
	...
	public double calculateAmountAccordingToPercent(double subTotal, double deliveryCost, double handlingCost, double percent) {
		...
		double calculatedAmount1=Math.round(calculatedAmount * 10000.0)/10000.0;
	...
	public void clickSaveAddNewPaymentProfilePopUPJs() {
		driver.scrollToElement(RFWebsiteDriver.driver,driver.findElement(PAYMENT_PROFILE_POPUP_SAVE_BUTTON_LOC));
		driver.pauseExecutionFor(2000);
		Actions action = new Actions(RFWebsiteDriver.driver);
		action.moveToElement(driver.findElement(PAYMENT_PROFILE_POPUP_SAVE_BUTTON_LOC)).click().build().perform();
		System.out.println("Clicked saved of payment profile via action");
		driver.waitForCSCockpitLoadingImageToDisappear();
		driver.pauseExecutionFor(3000);
	}
	...
	public void clickSaveAddNewPaymentProfilePopUPViaJs() {
		driver.scrollToElement(RFWebsiteDriver.driver,driver.findElement(PAYMENT_PROFILE_POPUP_SAVE_BUTTON_LOC));
		driver.pauseExecutionFor(2000);
	...
	public void clickOkButtonPaymentInfoAutoShipFailedOrderPopupInCheckoutTab(){
		if(driver.isElementPresent(PAYMENT_DETAILS_POPUP_OK_BTN)){
			driver.click(PAYMENT_DETAILS_POPUP_OK_BTN,"");
			driver.waitForCSCockpitLoadingImageToDisappear();
			driver.waitForElementToBeInVisible(PAYMENT_DETAILS_POPUP_OK_BTN, 3);
		}
	}

	public void selectSecondBillingAddress(String securityCode) {
		if(driver.isElementVisible(CREDIT_CARD_VERIFICATION_FAILED_ERROR_LOC)) {
			driver.click(BILLING_ADDRESS_DD_BTN_ON_ADD_NEW_BILLING_PROFILE_POPUP, "Select Billing Address Dropdown");
			driver.click(SECOND_BILLING_ADDRESS_VALUE_ON_ADD_NEW_BILLING_PROFILE_POPUP, "Second Billing Address");
			driver.pauseExecutionFor(2000);
			driver.type(SECURITY_CODE_TXT_FIELD_ON_ADD_NEW_BILLING_PROFILE_POPUP, securityCode);
	...
	public void clickAgainSaveAddNewPaymentProfilePopUPIfVisible() {
		if(driver.isElementVisible(PAYMENT_PROFILE_POPUP_SAVE_BUTTON_LOC)) {
			driver.clickByJS(RFWebsiteDriver.driver,PAYMENT_PROFILE_POPUP_SAVE_BUTTON_LOC,"");
			driver.waitForCSCockpitLoadingImageToDisappear();
	...
	public void enterBillingInfo(String profileName,String cardNumber,String cardType,String securityCode,String expMonth,String expYear){
		driver.pauseExecutionFor(2000);
		driver.type(CARD_NUMBER_TXT_FIELD_ON_ADD_NEW_BILLING_PROFILE_POPUP,cardNumber, "Card Number Text field on Add new Billing Profile Pop up");
		driver.type(NAME_ON_CARD_TXT_FIELD_ON_ADD_NEW_BILLING_PROFILE_POPUP, profileName, "Name on Card Text field on Add new Billing Profile Pop up");
		driver.click(CARD_TYPE_DD_BTN_ON_ADD_NEW_BILLING_PROFILE_POPUP,"Card Type Dropdown on Add New Billing Profile Pop up");
		driver.click(By.xpath("//td[text()='"+cardType+"']"),"Card Type");
		driver.click(EXPIRATION_MONTH_DD_BTN_ON_ADD_NEW_BILLING_PROFILE_POPUP,"Expiration Month dropdown on Add new Billing Profile pop up");
		driver.click(By.xpath("//td[text()='"+expMonth+"']"),"Expiration Month");
		driver.click(EXPIRATION_YEAR_DD_BTN_ON_ADD_NEW_BILLING_PROFILE_POPUP,"Expiration Year dropdown on Add new Billing Profile pop up");
		driver.click(By.xpath("//td[text()='"+expYear+"']"),"Expiration Year");
		driver.click(BILLING_ADDRESS_DD_BTN_ON_ADD_NEW_BILLING_PROFILE_POPUP,"Billing Address dropdown on Add new Billing Profile pop up");
		driver.click(BILLING_ADDRESS_VALUE_ON_ADD_NEW_BILLING_PROFILE_POPUP,"Billing Address value on Add new Billing Profile pop up");
		driver.type(SECURITY_CODE_TXT_FIELD_ON_ADD_NEW_BILLING_PROFILE_POPUP, securityCode, "Security Code Text field on Add new Billing Profile Pop up");
		driver.pauseExecutionFor(2000);
	}
	...
	public void enterShippingInfoInAddNewPaymentProfilePopupWithoutSaveBtn(String attendentFirstName,String addressLine,String city,String postalCode,String country,String province,String phoneNumber){
		driver.type(ATTENDENT_NAME_TEXT_BOX,attendentFirstName, "Attendent Name Text Box");
		driver.type(ADDRESS_LINE_TEXT_BOX,addressLine, "Address Line 1 Text box");
		driver.type(CITY_TOWN_TEXT_BOX_LOC, city, "City/Town Text Box");
		driver.type(POSTAL_TEXT_BOX, postalCode, "Postal Code Text Box");
		driver.type(COUNTRY_TEXT_BOX, country, "Country Text box");
		driver.doubleClick(By.xpath(String.format(selectOptionFromAutoCompleteList, country)), "Country from Auto Complete list");
		driver.pauseExecutionFor(2000);
		driver.type(PROVINCE_TEXT_BOX, province, "Province Text box");
		driver.doubleClick(By.xpath(String.format(selectOptionFromAutoCompleteList, province)), "State/Province(CA) or State/Territory(AU) from Auto Complete list");
		driver.pauseExecutionFor(2000);
		driver.type(PHONE_TEXT_BOX, phoneNumber, "Phone Number Text box");
		driver.waitForCSCockpitLoadingImageToDisappear();
	}
	...
	public void enterCVVValueInCheckoutTab(String CVV){
		...
		driver.click(CVV2_SEARCH_TXT_FIELD, "CVV field");
	...
	public void enterShippingDetailsInPopUpInCheckoutTab(String attendentFirstName,String attendeeLastName,String addressLine,String city,String postalCode,String Country,String province,String phoneNumber){
		driver.waitForElementToBeVisible(ATTENDENT_NAME_TEXT_BOX,30);
	...
	public void enterShippingInfoInAddNewPaymentProfilePopupWithoutSaveBtn(String attendentFirstName,String attendentLastName,String addressLine,String city,String postalCode,String Country,String province,String phoneNumber){
		driver.pauseExecutionFor(2000);
		driver.waitForElementToBeVisible(ATTENDENT_NAME_TEXT_BOX,20);
	...
	public void clickOrderTab(){
		driver.waitForElementPresent(ORDER_TAB);
		driver.clickByJS(RFWebsiteDriver.driver, ORDER_TAB,"");
		driver.waitForCSCockpitLoadingImageToDisappear();
	}

	
	public void clickAddToCartBtnInCartTab(){
		driver.click(ADD_TO_CART_BTN,"Add to cart button");
		driver.waitForCSCockpitLoadingImageToDisappear();
		driver.quickWaitForElementPresent(PRODUCT_NOT_AVAILABLE_POPUP_OK_BTN);
		boolean isProductFound = false;
		if(driver.isElementVisible(PRODUCT_NOT_AVAILABLE_POPUP_OK_BTN)==true){
			driver.click(PRODUCT_NOT_AVAILABLE_POPUP_OK_BTN,"product not available popup");
			clearCatalogSearchFieldAndClickSearchBtn();
			driver.waitForCSCockpitLoadingImageToDisappear();
			String totalNoOfPage = driver.findElement(TOTAL_NUMBER_OF_PAGE).getText().replaceAll("/", "").trim();
			int noOfPagesInSearchResult = Integer.parseInt(totalNoOfPage);
	...
	public void enterOrderNotesInCheckoutTab(String orderNote){
		...
		driver.type(ORDER_NOTES_TXT_FIELD, orderNote+" : Order Notes Text Field");
		driver.click(ADD_BTN,"Add BTN");
		driver.pauseExecutionFor(1000);
		driver.waitForCSCockpitLoadingImageToDisappear();
	}
	
	public boolean isProductSKUPresent(String catalogID){
		return driver.isElementPresent(By.xpath(String.format(prductNameWithSKU, catalogID)));
	}

	public String updateProductQuantityByAdd(String actualProductQuantity, String updateByCount){
		int updatedQuantity = Integer.parseInt(actualProductQuantity)+Integer.parseInt(updateByCount);
		logger.info("Updated quantity is "+updatedQuantity);
		return ""+updatedQuantity;
	}

	public String updateProductQuantityByMultiply(String actualProductQuantity, String updateByCount){
		if(actualProductQuantity.contains("$"))
			actualProductQuantity = actualProductQuantity.split("\\$")[1];
		double updatedQuantity = Double.parseDouble(actualProductQuantity)*Double.parseDouble(updateByCount);
		logger.info("Updated quantity by multiply is "+updatedQuantity);
		return ""+updatedQuantity;
	} 
	...
	public String convertUIDateFormatToPSTFormat(String UIDate){
		...
		String date, month, year = null;
		if (driver.getCountry().equalsIgnoreCase("au")) {
			splittedDate = UIDate.split("\\/");
			date = splittedDate[0];
	...
	public String convertPSTDateToNextDueDateFormat(String date){
		...
		String dueDate =  day+" "+UIMonth+","+" "+year;
		logger.info("Next due date format is "+dueDate);
		return dueDate;
	}
	
	public int getRandomCustomerFromSearchResult(){
		driver.waitForCSCockpitLoadingImageToDisappear();
		driver.waitForElementToBeVisible(TOTAL_CUSTOMERS_FROM_RESULT_FIRST_PAGE, 20);
		int totalCustomersFromResultsSearchFirstPage =  driver.findElements(TOTAL_CUSTOMERS_FROM_RESULT_FIRST_PAGE).size();
		logger.info("total customers in the customer search result is "+totalCustomersFromResultsSearchFirstPage);
	...
	public void clickOkButtonOfSelectPaymentDetailsPopupInCheckoutTab(){
		driver.click(PAYMENT_DETAILS_POPUP_OK_BTN,"Payment Details Popup OK Button");
		driver.waitForCSCockpitLoadingImageToDisappear();
		driver.waitForElementToBeInVisible(PAYMENT_DETAILS_POPUP_OK_BTN, 10);
		if(driver.isElementVisible(PAYMENT_DETAILS_POPUP_OK_BTN)){
			driver.click(PAYMENT_DETAILS_POPUP_OK_BTN,"Payment Details Popup OK Button");
			driver.waitForCSCockpitLoadingImageToDisappear();
			driver.waitForElementToBeInVisible(PAYMENT_DETAILS_POPUP_OK_BTN, 3);
		}
	...
	public void clickSaveAddNewPaymentProfilePopUP() {
		driver.waitForElementToBeClickable(PAYMENT_PROFILE_POPUP_SAVE_BUTTON_LOC, 10);
		driver.click(PAYMENT_PROFILE_POPUP_SAVE_BUTTON_LOC,"Payment Profile pop up Save button");
		driver.waitForCSCockpitLoadingImageToDisappear();
	}
	
	public boolean isCancelAutoshipBtnDisplayed(){
		return driver.isElementVisible(CANCEL_AUTOSHIP_BTN, 10);
	}
