...
public class CSCockpitOrderSearchTabPage extends CSCockpitRFWebsiteBasePage{
	...
	private static String orderNumberWithSequenceNumberLoc = "//div[@class='csListboxContainer']//div[@class='z-listbox-body']//tbody[2]/tr[%s]/descendant::a[1]";
	private static String orderNumberFromOrdersResultLoc = "//div[@class='csListboxContainer']//div[@class='z-listbox-body']//tr['%s']/td/div/a";
	...
	public void enterOrderNumberInOrderSearchTab(String orderNumber){
		driver.type(ORDER_NUMBER_TXT_FIELD_ORDER_SEARCH_TAB, orderNumber,"Order search text field");
	}
...
public void selectOrderStatusOnOrderSearchTab(String orderStatus){
		driver.click(ORDER_STATUS_DD,"Order Status Dropdown");
		driver.click(By.xpath(String.format(orderStatusOptionLoc, orderStatus)),"Order Status option");
		driver.waitForCSCockpitLoadingImageToDisappear();
	}
	
public void enterCIDOnOrderSearchTab(String CID){
		driver.type(CID_CUSTOMER_NAME_TXT_FIELD, CID, "CID or Customer Name");
		driver.waitForCSCockpitLoadingImageToDisappear();
	}
	
public void clickOrderNumberInOrderSearchResultsInOrderSearchTab(String orderNumber){
	String number=null;
	if(Integer.parseInt(orderNumber)>20){
		number=orderNumber;
	}
	else{
		number= driver.getText(By.xpath(String.format(orderNumberWithSequenceNumberLoc, orderNumber))).trim();
		driver.pauseExecutionFor(1000);	
	}
	driver.pauseExecutionFor(1000);
	driver.waitForElementPresent(By.xpath(String.format(orderNumberFromOrdersResultLoc, number)), 30);
	driver.click(By.xpath(String.format(orderNumberFromOrdersResultLoc,number)),"Order Number");
	driver.waitForCSCockpitLoadingImageToDisappear();
}
...
public String getAndclickOrderNumberInOrderSearchResultsInOrderSearchTab(String orderSequenceNumber){
	String number=null;
	if(Integer.parseInt(orderSequenceNumber)>20){
		number=orderSequenceNumber;
	}
	else{
		number= driver.getText(By.xpath(String.format(orderNumberWithSequenceNumberLoc, orderSequenceNumber))).trim();
		driver.pauseExecutionFor(1000);	
	}
	driver.pauseExecutionFor(1000);
	driver.waitForElementPresent(By.xpath(String.format(orderNumberFromOrdersResultLoc, number)), 30);
...
public int getRandomOrdersFromOrderResultSearchFirstPageInOrderSearchTab(){
	driver.waitForCSCockpitLoadingImageToDisappear();
	driver.waitForElementToBeVisible(TOTAL_ORDERS_FROM_ORDER_SEARCH_RESULT, 10);
