...
public class CSCockpitCustomerTabPage extends CSCockpitRFWebsiteBasePage{
	...
	private static String orderSectionLoc ="//*[contains(text(),'%s')]";	
	...
	private static String shippingProfileName = "//div[text()='%s']/ancestor::tr[1]//td[text()=\"Edit\"]";

	private static final By SET_AS_AUTOSHIP_SHIPPING_PROFILE_TEXT = By.xpath("//span[contains(text(),'Set as a Autoship Shipping Address')]");
	private static final By USE_THIS_ADDRESS = By.xpath("//td[contains(text(),'Use this Address') or contains(text(),'Use this address')]");	
	...
	public void clickPlaceOrderButtonInCustomerTab(){
		...
		driver.click(PLACE_ORDER_BUTTON,"place order button");
	}
	...
	public String convertPulseTemplateDate(String UIDate){
		...
		if(driver.getCountry().equalsIgnoreCase("au")){
			System.out.println("Date is "+date+" "+UIMonth+" "+"20"+year);
			return date+" "+UIMonth+" "+"20"+year;
		}else{
			System.out.println("Date is "+UIMonth+" "+date+", "+"20"+year);
			return UIMonth+" "+date+", "+"20"+year;
		}}
	...
	public String getFirstShippingAddressProfileName(){
		...
		String firstname = driver.getText(SHIPPING_ADDRESS_PROFILE_FIRST_NAME);
		String lastname = driver.getText(SHIPPING_ADDRESS_PROFILE_LAST_NAME);
	...
	public boolean isAutoshipIdWithPendingCRPPresent(){
		return driver.isElementPresent(AUTOSHIP_ID_HAVING_TYPE_AS_CRP_AUTOSHIP_STATUS_AS_PENDING, 5);
	}
	...
	public boolean isAutoshipIdWithPendingPCPerksPresent(){
		return driver.isElementPresent(AUTOSHIP_ID_HAVING_TYPE_AS_PC_AUTOSHIP_STATUS_AS_PENDING, 5);
	}
...
public boolean isEditButtonForCreditCardInCustomerTabPresent(){
	driver.waitForElementPresent(CREDIT_CARD_EDIT_BTN);
	driver.pauseExecutionFor(5000);
	return driver.isElementPresent(CREDIT_CARD_EDIT_BTN);
}
...
public String getEmailAddressFromTopSectionInCustomerTabPage(){
	return driver.getText(By.xpath("//div[@class='csObjectCustomerContainer']//span[contains(text(),'Email')]/following-sibling::span[1]"), "Email address from Top section");
}

public String getUserNameAndCIDStringFromTopSectionInCustomerTabPage(){
	return driver.getText(By.xpath("//div[@class='csObjectCustomerContainer']/descendant::span[contains(@class,'csCustomerNameLabelValue')][1]"), "Username and CID from Top section");
}

public void clickEditBtnForTheBillingProfile(String billingProfile){
	driver.click(By.xpath("//div[contains(text(),'"+billingProfile+"')]/following::td[text()='EDIT'][1]"),"Edit link of Billing Profile");
}

public void clickCreateNewAddressBtn(){
	driver.click(CREATE_NEW_ADDRESS_IN_SHIPPING_ADDRESS_POPUP,"Create New Address Button in Shipping Address Pop up");
	driver.waitForCSCockpitLoadingImageToDisappear();
}

public void clickUseEnteredAddressBtn(){
	driver.click(USE_ENTERED_ADDRESS,"Use ad Entered button");
	driver.waitForCSCockpitLoadingImageToDisappear();
}

public void clickEditButtonOfShippingAddressInCustomerTab(Object randomNumber){
	String editLinkforShippingProfile = String.format(shippingProfileName, randomNumber);
	driver.click(By.xpath(editLinkforShippingProfile),"Edit link for Shipping Profile");
	driver.waitForCSCockpitLoadingImageToDisappear();
}
...
public String getNextDueDateOfCRPAutoshipAndStatusIsPending(){
	...
	return driver.getText(NEXT_DUE_DATE_OF_AUTOSHIP_TEMPLATE,"Next due date at autoship template");
			}



public void clickUpdateAddressBtn(){
	driver.click(UPDATE_ADDRESS_IN_SHIPPING_ADDRESS_POPUP,"Update address Button in Shipping Profile Pop up");
	driver.waitForCSCockpitLoadingImageToDisappear();
}

public boolean isShippingProfilePresent(Object randomNumber){
	String shippingProfileNamme = String.format(shippingProfileName, randomNumber);
	return driver.isElementPresent(By.xpath(shippingProfileNamme),20);
}

public boolean isBillingListContainsBillingName(String billingProfileName){
	return driver.isElementPresent(By.xpath(String.format("//span[text()='Billing Information']/following::tr[contains(@class,'csListItem')]/td[2]/div[contains(text(),'%s')]",billingProfileName)),20);
}

public String getAndClickAutoshipIDHavingTypeAsCRPAutoshipAndStatusIsPending(){
	driver.waitForElementPresent(AUTOSHIP_ID_HAVING_TYPE_AS_CRP_AUTOSHIP_STATUS_AS_PENDING, 20);
	String autoshipID = driver.getText(AUTOSHIP_ID_HAVING_TYPE_AS_CRP_AUTOSHIP_STATUS_AS_PENDING,"Autoship id from CS cockpit UI");
...
public void clickAddButtonOfCustomerAddressInCustomerTab(){
	driver.click(ADD_NEW_SHIPPING_ADDRESS,"Add New at Shipping Profiles");
	driver.waitForCSCockpitLoadingImageToDisappear();
}

public void clickOnSetAsAutoshipBillingProfileChkBox(){
	driver.waitForElementToBeVisible(SET_AS_AUTOSHIP_BILLING_PROFILE_CHK_BOX,10);
	driver.click(SET_AS_AUTOSHIP_BILLING_PROFILE_CHK_BOX,"");
	driver.waitForCSCockpitLoadingImageToDisappear();
} 

public String getOrderTypeInCustomerTab(String orderNumber){
	driver.waitForElementToBeVisible(By.xpath(String.format(orderTypeLoc, orderNumber)),20);
...
public void clickAddCardButtonInCustomerTab(){
	driver.waitForElementToBeVisible(ADD_CARD_BTN, 30);
	driver.click(ADD_CARD_BTN,"Add Card Button in customer tab");
	driver.waitForCSCockpitLoadingImageToDisappear();
}
