...
public class CSCockpitOrderTabPage extends CSCockpitRFWebsiteBasePage{
	...
	private static String prductDetailsFromSKU =  "//span[contains(text(),'%s')]/ancestor::tr[1]/descendant::td[%s]//input";
	private static String BasePriceFromSKU =  "//span[contains(text(),'%s')]/ancestor::tr[1]/descendant::td[4]//div";
	...
	private static final By CHANGE_SPONSER_LINK = By.xpath("//span[contains(text(),'Consultant Receiving Commissions')]/following-sibling::a[text()='Change']");
	...
	private static final By NO_REFUNDABLE_FIRST_PRODUCT_CHKBOX = By.xpath("//div[contains(text(),'Product Info')]/ancestor::div[@class='z-listbox-header']/following::div[1]//tbody[2]/tr[1]/td[1]//input");
	private static final By REFUND_TOTAL_LBL = By.xpath("//span[text()='REFUND TOTAL' or text()='Refunded Grand Total']/following::span[1]");
	private static final By REFUND_TAX_LBL = By.xpath("//span[text()='Refunded Tax' or text()='Includes Refunded GST of']/following::span[1]");
	private static final By CV_VALUE = By.xpath("//span[text()='Total CV:']/following::span[1]");
	private static final By QV_VALUE = By.xpath("//span[text()='Total QV:']/following::span[1]");
	...
	public String getOrderNumberFromCsCockpitUIOnOrderTab() {
		...
		String orderNumberOnOrderTab = driver.getText(ORDER_TITLE_CSCOCKPIT_UI_LOC,"order title");
	...
	public boolean isNoRefundableProductPresent(){
		driver.waitForElementPresent(NO_REFUNDABLE_FIRST_PRODUCT_CHKBOX);
		return !driver.isElementPresent(NO_REFUNDABLE_FIRST_PRODUCT_CHKBOX);
	}

	public boolean areAllCheckBoxesGettingDisabledAfterCheckingReturnCompleteOrderChkBox(){
		Boolean allDisableCheckbox = false;
		if(driver.getCountry().equalsIgnoreCase("au")){
			driver.waitForElementPresent(DISABLED_PRODUCT_CHKBOX_REFUND_POPUP);
			allDisableCheckbox= driver.isElementPresent(DISABLED_PRODUCT_CHKBOX_REFUND_POPUP)&&
		...
		return allDisableCheckbox;
	}

	public void checkReturnTaxOnlyCheckboxInPopUp(String rowNum) {
		if(driver.getCountry().equalsIgnoreCase("au")){
			logger.info("return tax only checkbox not present.");
		}else{
	...
	public boolean isQVandCVUpdateBtnEnabledOnOrderTab(){
		driver.waitForElementPresent(CV_QV_UPDATE_BTN);
		return driver.findElement(CV_QV_UPDATE_BTN).isEnabled();
	}
	...
	public String getCVValue(){
		driver.waitForElementPresent(CV_VALUE);
		String CVValue = driver.getText(CV_VALUE,"CV value is");
		return CVValue;
	}

	public String getQVValue(){
		driver.waitForElementPresent(QV_VALUE);
		String QVValue = driver.getText(QV_VALUE,"QV value is");
		return QVValue;
	}

	public String getProductDetailsFromSKU(String SKU, String columnName){
		if(columnName.equalsIgnoreCase("cv"))
			columnName = "7";
		else if(columnName.equalsIgnoreCase("qv"))
			columnName = "8";
		driver.waitForElementPresent(By.xpath(String.format(prductDetailsFromSKU, SKU,columnName)));
		String value = driver.getAttribute(By.xpath(String.format(prductDetailsFromSKU, SKU,columnName)),"value");
		return value;
	}

	public String getBasePriceFromSKU(String SKU){
		driver.waitForElementPresent(By.xpath(String.format(BasePriceFromSKU, SKU)),"For Base Price");
		String value = driver.getText(By.xpath(String.format(BasePriceFromSKU, SKU)),"base price is");
		return value.replaceAll(",","");
	}

	public boolean isOrderPlacedSuccessfully(){
		return driver.isElementVisible(ORDER_NUMBER);
	} 
	
	public void clickCreateBtnOnRefundPopUp(){
		driver.waitForElementToBeVisible(CREATE_BTN_REFUND_POPUP,10);
