...
public class CSCockpitCheckoutTabPage extends CSCockpitRFWebsiteBasePage{
	...
	private static String addressLineTextBoxLoc = "//span[text()='%s']/following::input[1]";
	
	private static final By TOTAL_PRICE_TXT_TOTALS_SECTION = By.xpath("//span[contains(text(),'Total Price') or contains(text(),'Grand Total')]");
	private static final By POSTAL_CODE_TEXT_BOX = By.xpath("//span[contains(text(),'Post')]/following::input[1]");
	...
	private static final By DELIVERY_MODE_AU = By.xpath("//span[contains(text(),'Delivery Mode')]/following::select[@z.onselect='true']/option");
	...
	public String getCreditCardNumberInCheckoutTab(){		
		...
		if(driver.isElementPresent(CREDIT_CARD)==false){
			...
			driver.type(CARD_NUMBER_TXT_FIELD_ON_ADD_NEW_BILLING_PROFILE_POPUP, TestConstants.CARD_NUMBER,"card number");
			driver.type(NAME_ON_CARD_TXT_FIELD_ON_ADD_NEW_BILLING_PROFILE_POPUP, TestConstants.NEW_BILLING_PROFILE_NAME,"billing profile name");
			driver.click(CARD_TYPE_DD_BTN_ON_ADD_NEW_BILLING_PROFILE_POPUP,"card type dropdown");
			driver.click(CARD_TYPE_VALUE_VISA_ON_ADD_NEW_BILLING_PROFILE_POPUP,"Visa card");
			driver.click(EXPIRATION_MONTH_DD_BTN_ON_ADD_NEW_BILLING_PROFILE_POPUP,"Exp month dropdown");
			driver.click(EXPIRATION_MONTH_VALUE_ON_ADD_NEW_BILLING_PROFILE_POPUP,"Exp month value");
			driver.click(EXPIRATION_YEAR_DD_BTN_ON_ADD_NEW_BILLING_PROFILE_POPUP,"Exp year dropdown");
			driver.click(EXPIRATION_YEAR_VALUE_ON_ADD_NEW_BILLING_PROFILE_POPUP,"Exp year value");
			driver.type(SECURITY_CODE_TXT_FIELD_ON_ADD_NEW_BILLING_PROFILE_POPUP, TestConstants.SECURITY_CODE,"security code as"+TestConstants.SECURITY_CODE);
			driver.click(BILLING_ADDRESS_DD_BTN_ON_ADD_NEW_BILLING_PROFILE_POPUP,"Billing address dropdown");
			driver.click(BILLING_ADDRESS_VALUE_ON_ADD_NEW_BILLING_PROFILE_POPUP,"Billung address value");
			driver.click(SAVE_BTN_ON_ADD_NEW_BILLING_PROFILE_POPUP,"Save button");
			driver.waitForCSCockpitLoadingImageToDisappear();
		}
	...
	public void clickPlaceOrderButtonInCheckoutTab(){
		driver.click(PLACE_ORDER_BUTTON_CHECKOUT_TAB,"place order button");
		driver.waitForCSCockpitLoadingImageToDisappear();
	...
	public void clickCloseOfPaymentAddressPopUpInCheckoutTab(){
		...
		driver.waitForElementToBeInVisible(CLOSE_POPUP_OF_PAYMENT_ADDRESS, 5);		
		driver.waitForCSCockpitLoadingImageToDisappear();
	...
	public void clickCloseOfDeliveryAddressPopUpInCheckoutTab(){
		...
		driver.waitForCSCockpitLoadingImageToDisappear();
		driver.waitForElementToBeInVisible(CLOSE_POPUP_OF_DELIVERY_ADDRESS, 5);
	}
	...
	public void clickUseAsEnteredPopupOkayInCheckoutTab(){
		...
		driver.waitForElementToBeInVisible(USE_AS_ENTERED_POPUP, 5);
		driver.waitForCSCockpitLoadingImageToDisappear();
...
public boolean verifyAddressTextBoxInPopUpInCheckoutTab(String value){
	driver.quickWaitForElementPresent(By.xpath(String.format(addressLineTextBoxLoc, value)));
	logger.info("Address line 1 name is: "+value);
	return driver.isElementPresent(By.xpath(String.format(addressLineTextBoxLoc, value)));
}

public String getDeliverModeTypeInCheckoutTab(){
	if(driver.getCountry().equalsIgnoreCase("au")){
		return driver.findElement(DELIVERY_MODE_AU).getText();
	}else{
...
public void selectOverrideReasonSooReason(){
	driver.waitForElementToBeVisible(PERFORM_SOO_REASON_LOC,10);
	driver.click(PERFORM_SOO_REASON_LOC,"PERFORM_SOO_REASON_LOC");
	logger.info("SOO REASON DD CLICKED IN SALES OVERRIDE POPUP");
	driver.waitForElementToBeVisible(SELECT_SOO_REASON_AS_WAIVE_LOC,10);
	driver.click(SELECT_SOO_REASON_AS_WAIVE_LOC,"SELECT_SOO_REASON_AS_WAIVE_LOC");
	logger.info("SOO REASON DD VALUE SELECTED");
	driver.waitForLoadingImageToDisappear();
}

public void clickUpdateButtonSalesOverridePopUp(){
	driver.waitForElementToBeVisible(SALES_OVERRIDE_POPUP_UPDATE_BUTTON_LOC,10);
	driver.click(SALES_OVERRIDE_POPUP_UPDATE_BUTTON_LOC,"SALES_OVERRIDE_POPUP_UPDATE_BUTTON_LOC");
