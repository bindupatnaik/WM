...
public class CRPAutoshipVerificationTest extends RFWebsiteBaseTest{
	...
	public void testVerifyAddNewPaymentProfileFunctionalityInCRPCheckout_1816() throws InterruptedException{
		...
		cscockpitAutoshipTemplateUpdateTabPage.clickSaveAddNewPaymentProfilePopUPViaJs();
		s_assert.assertTrue(cscockpitCheckoutTabPage.getNewBillingAddressNameInCheckoutTab().contains(attendeeFirstName),"CSCockpit checkout tab newly created billing address name expected ="+ attendeeFirstName+ "and on UI = " +cscockpitCheckoutTabPage.getNewBillingAddressNameInCheckoutTab());
	...
	public void testVerifyCancelAutoshipButtonNotDisplayedInPCCheckoutPage_1713() throws InterruptedException{
		...
		//		s_assert.assertFalse(storeFrontPCUserPage.isEditCRPLinkPresent(), "Edit CRP link is present");
	...
	public void testVerifyAddCardFunctionalityInCRPEditAutoshipTemplate_1741(){
		...
		System.out.println("profile name 1 is "+profileName);
		...
		System.out.println("profile name 2 is "+profileName);
		...
		cscockpitAutoshipTemplateTabPage.clickSaveAddNewPaymentProfilePopUPJs();
	...
	public void testVerifyChangeCRPDate_1705() throws InterruptedException{
		...
		//		}else {
		//			s_assert.assertTrue(crpDate.trim().split("\\ ")[0].contains(day.split("\\,")[0]), "Expected next day of CRP is "+day.split("\\,")[0]+"Actual on UI in storeFront "+crpDate);
		//		}
		//
		//		logout();
		s_assert.assertAll();
	...
	public void testVerifyCreateCRPAutoship_1700() throws InterruptedException{
		...
		if(isProductPresent==true){
			...
			s_assert.assertTrue(cscockpitAutoshipTemplateTabPage.verifyThresholdPopup(),"Threshold popup does not appear");
			cscockpitAutoshipTemplateTabPage.clickOKOfThresholdPopupInAutoshipTemplateTab();
		}
		...
		s_assert.assertTrue(cscockpitAutoshipTemplateTabPage.isCancelAutoshipBtnDisplayed(), "Autoship not created successfully");
		
//		String dueDate = cscockpitAutoshipTemplateTabPage.getNextDueDateOfCRPAutoship();//12/15/17
...
//		s_assert.assertTrue(oneMonthExtendedDate.trim().contains(nextDueDate), "Expected next due date of CRP is "+nextDueDate+"Actual on UI in Cscockpit "+oneMonthExtendedDate.trim());
		...
		//		}else {
		//			s_assert.assertTrue(crpDate.trim().contains(oneMonthExtendedDate.trim().split(" ")[1].split("\\,")[0]), "Expected next due date of CRP is "+oneMonthExtendedDate+"Actual on UI in storeFront "+crpDate);
		//		}
		//
		//		logout();
		s_assert.assertAll();
	...
	public void testVerifyDelayPCPerks_1709() throws InterruptedException{
		...
		String dueDateWithCompleteYear =null;
		...
		if(driver.getCountry().equalsIgnoreCase("au"))
			dueDateWithCompleteYear = nextDueDate.split("\\/")[0]+"/"+nextDueDate.split("\\/")[1]+"/"+nextDueDate.split("\\/")[2];
		else
		...
		//			}else {
		//				s_assert.assertTrue(dueDate.trim().contains(delayAutoshipDateInMonthForm.trim().split("\\ ")[1].split("\\,")[0]), "Expected delayed autoship date is "+delayAutoshipDateInMonthForm+" Actual On store front "+dueDate);
		//			}
		//
		//			logout();
		s_assert.assertAll();
	...
	public void testVerifyCreatePaymentAddressFunctionalityInCRPCheckoutPage_1736() throws InterruptedException{
		...
		cscockpitAutoshipTemplateUpdateTabPage.enterShippingInfoInAddNewPaymentProfilePopupWithoutSaveBtn(attendentFirstName, attendeeLastName, addressLine1, city, postalCode, countryDDValue, state, phoneNumber);
		cscockpitAutoshipTemplateUpdateTabPage.clickSaveAddNewPaymentProfilePopUP();
		s_assert.assertTrue(cscockpitAutoshipTemplateUpdateTabPage.isReviewCreditCardDetailsErrorMsgPresent(), "Review credit card error msg is not present");
		cscockpitAutoshipTemplateUpdateTabPage.enterCreditCardNumberInPaymentProfilePopup();
		cscockpitAutoshipTemplateUpdateTabPage.enterCVVNumberInPaymentProfilePopup();
		cscockpitAutoshipTemplateUpdateTabPage.clickSaveAddNewPaymentProfilePopUPJs();
		cscockpitAutoshipTemplateUpdateTabPage.enterCVVValueInCheckoutTab(TestConstants.SECURITY_CODE);
		cscockpitAutoshipTemplateUpdateTabPage.clickUseThisCardBtnInCheckoutTab();
		cscockpitAutoshipTemplateUpdateTabPage.clickUpdateAutoshipTemplateInAutoshipTemplateUpdateTab();
		//		driver.get(driver.getStoreFrontURL()+"/"+driver.getCountry());
	...
	public void testDelayPCPerksAutoshipTemplateBy30Days_4783() throws InterruptedException{
		...
		String pcEmailID = null;
		String countryDDValue = null;
		String delayAutoshipDate=null;
		String delayAutoshipDateInMonthForm=null;
		String dueDate =null;
		String noOfDays = "30";
		List<Map<String, Object>> randomPCList =  null;
		if(driver.getCountry().equalsIgnoreCase("ca")){
	...
	//Hybris Project-1746:To verify Add New Shipping address functionality on PCPerk Autoship template Page
	@Test
	public void testVerifyAddNewShippingAddressFunctionalityOnPCPerks_1746(){
		...
		String pcEmailID = null;
		String accountID = null;
		String defaultSuggestionSelected =null;
		String getNewShippingAddressLineFromUI = null;
		String getShippingAddressLineFromUI =null;
		String randomCustomerSequenceNumber = null;
		List<Map<String, Object>> randomPCList =  null;
		List<Map<String, Object>> randomPCUsernameList =  null;
		String addressLine1 = "54th ST";
		...
		if(driver.getCountry().equalsIgnoreCase("ca")){
			...
			countryDDValue= TestConstants.COUNTRY_DD_VALUE_AU;
			addresstoPass=TestConstants.POSTAL_CODE_AU;
		}
		...
		cscockpitAutoshipTemplateTabPage.enterShippingInfoInAddNewPaymentProfilePopupWithoutSaveBtn(attendentFirstName, attendeeLastName, addressLine1, city, postalCode, countryDDValue, state, phoneNumber);
		cscockpitAutoshipTemplateTabPage.clickCreateNewAddressButtonInPopupAutoshipTemplateTabPage();
		...
		getShippingAddressLineFromUI=cscockpitAutoshipTemplateTabPage.getShippingAddressLine1UnderShippingAddress();
		s_assert.assertTrue(getShippingAddressLineFromUI.equalsIgnoreCase(addressLine1),"Newly created shipping address is not selected from dropdown");
		cscockpitAutoshipTemplateTabPage.clickAddNewAddressButtonUnderShippingAddress();
		cscockpitAutoshipTemplateTabPage.enterShippingInfoInAddNewPaymentProfilePopupWithoutSaveBtn(attendentNewFirstName, attendeeLastName, addressLine1, city, postalCode, countryDDValue, state, phoneNumber);
		cscockpitAutoshipTemplateTabPage.clickCreateNewAddressButtonInPopupAutoshipTemplateTabPage();
		s_assert.assertTrue(cscockpitAutoshipTemplateTabPage.isQASpopupPresent(), "QAS popup is not present while creating  new shipping address");
		//	cscockpitAutoshipTemplateTabPage.clickUseEnteredAddressOfCreateNewAddressShippingAddressPopup();
		defaultSuggestionSelected =cscockpitAutoshipTemplateTabPage.selectAndReturnAddressFromDropdownBeforeClickingUseThisAddressBtnInPopup(addresstoPass);;
		cscockpitAutoshipTemplateTabPage.clickButtonsInDeliveryAndShippingAddressPopup("Use this address");
		getNewShippingAddressLineFromUI=cscockpitAutoshipTemplateTabPage.getShippingAddressLine1UnderShippingAddress();
	...
	public void testVerifyOnlyUSProductIsDisplayedOnPCPerkCartPage_4794(){
		...
		if(driver.getCountry().equalsIgnoreCase("CA")) {
			...
			s_assert.assertTrue(cscockpitAutoshipCartTabPage.getSelectProductCatalogValue().contains("Australia Product Catalog"),"Expect name of options is Candada Product Catalog but actual on UI "+cscockpitAutoshipCartTabPage.getSelectProductCatalogValue());	
		}
	...
	public void testVerifyEditCardFunctionalityInCRPEditAutoshipTemplatePage_1743(){
		...
		profileName = TestConstants.NEW_BILLING_PROFILE_NAME+randomNum2;
		cscockpitAutoshipTemplateTabPage.enterBillingInfoWithoutSelectAddress(profileName);
		cscockpitAutoshipTemplateTabPage.enterShippingInfoInAddNewPaymentProfilePopupWithoutSaveBtn(attendentFirstName, attendeeLastName, addressLine1, city, postalCode, country, state, phoneNumber);
		cscockpitAutoshipTemplateTabPage.clickSaveAddNewPaymentProfilePopUP();
		s_assert.assertTrue(cscockpitAutoshipTemplateTabPage.getPaymentAddressNameInAutoshipTemplateTab().toLowerCase().trim().contains(profileName.toLowerCase().trim()),"Payment Address Name Expected"+profileName+" but on UI"+cscockpitAutoshipTemplateTabPage.getPaymentAddressNameInAutoshipTemplateTab());
		s_assert.assertAll();
	}
	...
	public void testVerifyEditCardFunctionalityInPCPerksEditAutoshipTemplatePage_1744(){
		...
		if(driver.getCountry().equalsIgnoreCase("ca")){
			...
			countryDDValue= TestConstants.COUNTRY_DD_VALUE_AU;
			line1 = line1 = TestConstants.LINE1_AU;;
		}
		List<Map<String, Object>> randomPCList = null;
		//				storeFrontHomePage = new StoreFrontHomePage(driver);
		//				driver.get(driver.getStoreFrontURL()+"/"+driver.getCountry());
