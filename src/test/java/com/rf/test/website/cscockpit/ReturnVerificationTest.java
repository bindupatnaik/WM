...
public class ReturnVerificationTest extends RFWebsiteBaseTest{
	...
	@Test(enabled=false) // don't have csCommisionAdminUserName
...
//		while(true){
			...
			randomRCEmailIDList = DBUtil.performDatabaseQuery(DBQueries_RFO.callQueryWithArguement(DBQueries_RFO.GET_EMAIL_ID_FROM_ACCOUNT_ID,accountID),RFO_DB);
			rcUserEmailID = String.valueOf(getValueFromQueryResult(randomRCEmailIDList, "EmailAddress"));  
			logger.info("emaild of rc username "+rcUserEmailID);
...
//		driver.get(driver.getCSCockpitURL());		
		//Login with CS commission admin credentials.
	...
	public void testToVerifyThatForTerminatedUserCanDoReturns_2013(){
		...
		s_assert.assertTrue(cscockpitOrderTabPage.verifyRefundOrderButtonPresentOnOrderTab(),"Refund order button is not present on order tab ");
		s_assert.assertAll();
	}
	...
	public void testToVerifyTheRefundRequestPopUpUI_2014(){
		...
		if(driver.getCountry().equalsIgnoreCase("ca")){
			s_assert.assertTrue(cscockpitOrderTabPage.verifyOrderLevelCheckBoxSection(),"order level checkBoxes not present as expected");
		}
		s_assert.assertTrue(cscockpitOrderTabPage.verifyRefundReasonDDPresent(),"Refund Reason DD not present on pop up ");
		s_assert.assertTrue(cscockpitOrderTabPage.verifyCreditCardDDPresent(),"credit card DD not present on pop up ");
		s_assert.assertTrue(cscockpitOrderTabPage.verifyRefundTypeDDPresent(),"Refund Type DD not present on pop up ");
...
//			driver.get(driver.getCSCockpitURL());		
			...
			if(driver.getCountry().equalsIgnoreCase("ca")){
				...
				s_assert.assertFalse(cscockpitOrderTabPage.getShippingAndHandlingVariousSectionTaxInPopup(refundShipping).contains("14.00"),"Refund shipping expected on UI for US "+14.00+"while Actual on UI is "+cscockpitOrderTabPage.getShippingAndHandlingVariousSectionTaxInPopup(refundShipping));	
			}
...
//			logout();
	...
	public void testVerifyReturnHandlingFunctionality_2025() throws InterruptedException{
		if(driver.getCountry().equalsIgnoreCase("ca")){
			...
			s_assert.assertTrue(cscockpitOrderTabPage.getShippingAndHandlingVariousSectionTaxInPopup(refundSubtotal).contains("0.00"),"Refund subtotal expected on UI for us "+0.00 +"while Actual on UI is "+cscockpitOrderTabPage.getShippingAndHandlingVariousSectionTaxInPopup(refundSubtotal));
			s_assert.assertTrue(cscockpitOrderTabPage.getShippingAndHandlingVariousSectionTaxInPopup(refundTax).contains("0.00"),"Refund tax expected on UI for us "+0.00 +"while Actual on UI is "+cscockpitOrderTabPage.getShippingAndHandlingVariousSectionTaxInPopup(refundTax));
	...
	public void testVerifyPartiallyReturnFunctionality_2028() throws InterruptedException{
		if(driver.getCountry().equalsIgnoreCase("ca")){
			...
			s_assert.assertTrue(cscockpitOrderTabPage.getShippingAndHandlingVariousSectionTaxInPopup(refundHandling).contains("0.00"),"Refund Handling expected on UI for US "+2.50+"while Actual on UI is "+cscockpitOrderTabPage.getShippingAndHandlingVariousSectionTaxInPopup(refundHandling));
	...
	@Test//Partial return only for CA
...
//			cscockpitOrderSearchTabPage.clickOrderNumberInOrderSearchResultsInOrderSearchTab(randomOrderSequenceNumber);
			cscockpitOrderTabPage.clickOrderTab();
			cscockpitOrderTabPage.clickRefundOrderBtnOnOrderTab();
			cscockpitOrderTabPage.checkReturnTaxOnlyCheckboxInPopUp("2");
			cscockpitOrderTabPage.selectRefundReasonOnRefundPopUp("Test");
			cscockpitOrderTabPage.selectFirstReturnActionOnRefundPopUp();
