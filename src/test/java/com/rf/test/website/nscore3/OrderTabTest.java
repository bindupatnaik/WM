...
public class OrderTabTest extends RFNSCoreWebsiteBaseTest{
	private static final Logger logger = LogManager
			.getLogger(OrderTabTest.class.getName());

	private NSCore3HomePage nscore3HomePage;
	private NSCore3LoginPage nscore3LoginPage;
	private String RFL_DB = null;

	//QTest ID TC-221 Orders - User is able to search for orders by customer name
	@Test 
	public void testOrdersUserIsAbleToSearchForOrdersByCustomerName_221(){
	...
	//QTest ID TC-235 Distributors-Search by e-mail
	@Test 
	public void testSearchByEmail_235() throws InterruptedException{
		RFL_DB = driver.getDBNameRFL();
	...
	//QTest ID TC-224 Orders - User is able to search for orders by order number
	@Test 
	public void testUserAbleToSearchOrdersByOrderNumber_224(){
		String orderID = "Order ID";
		String orderNumber = "Order Number";
		String customer = "Customer";
		String subTotal = "Subtotal";
...
//		s_assert.assertTrue(nscore3HomePage.isExpectedValuePresentInColumn(customerName, consultantColumnNumber), customerName+" NOT found in search result");
	...
	//QTest ID TC-222 Orders - User is able to search for orders by customer account
	@Test 
	public void testVerifyUserIsAbleToSearchForOrderByCustomerAccount_222() throws InterruptedException{
		RFL_DB = driver.getDBNameRFL();
		String orderID = "Order ID";
		String orderNumber = "Order Number";
		String customer = "Customer";
	...
	//QTest ID TC-232 Distributors-Search by customer Name
	@Test 
	public void testSearchByCustomerName_232() throws InterruptedException{
		RFL_DB = driver.getDBNameRFL();
		String distributorTab = "Distributors";
	...
	//QTest ID TC-233 Distributors-Search by consultant
	@Test 
	public void testSearchByConsultant_233() throws InterruptedException{
		RFL_DB = driver.getDBNameRFL();
		String distributorTab = "Distributors";
	...
	//QTest ID TC-231 Distributors-search by account No
	@Test 
	public void testDistributorsbyAccountNo_231(){
		RFL_DB = driver.getDBNameRFL();
		...
		nscore3HomePage.clickSearchOnDistributorTab();
		s_assert.assertTrue(nscore3HomePage.getTotalSearchResultDisplayed()>0,"Search results not displayed when searched with account number");
...
//		s_assert.assertTrue(sponsorFromUI.contains(sponsorAccountNo), "Expected sponsor id is: "+sponsorAccountNo+" Actual on UI: "+sponsorFromUI);
	...
	//QTest ID TC-230 Distributors-Search by sponsor name
	@Test 
	public void testSearchBySponserName_230() throws InterruptedException{
		RFL_DB = driver.getDBNameRFL();
		String distributorTab = "Distributors";
		...
		nscore3HomePage.clickSearchOnDistributorTab();
		s_assert.assertTrue(nscore3HomePage.getTotalSearchResultDisplayed()>0,"Search results not displayed when searched with name");
...
//		s_assert.assertTrue(cellValue.equalsIgnoreCase(emailAddress)," Expected Customer email address "+emailAddress+" is not present in row and column"+emailIdColumnNumber+" While actual on UI is: "+cellValue);
	...
	//QTest ID TC-1900 Run pulse
	@Test 
	public void testRunPulse_1900(){
		RFL_DB = driver.getDBNameRFL();
		String customerName = null;
		String accountNumber = null;
		String firstName = null;
		String lastName = null;
		String autoshipOrderType = "Pulse";
		String category="Name";
		String errorMsg="Failed! Template already processed this month";
		List<Map<String, Object>> randomAccountList =  null;
		...
		nscore3HomePage.selectSearchByCategoryOptionOnAutoshipOrdersPage(category);
		...
		nscore3HomePage.clickDashboardTab();
		nscore3HomePage.clickOrdersLink();
		nscore3HomePage.clickGoToAutoshipOrdersLink();
		nscore3HomePage.selectAutoshipOrderType(autoshipOrderType);
		nscore3HomePage.selectSearchByCategoryOptionOnAutoshipOrdersPage(category);
		...
		nscore3HomePage.selectRunCheckbox(accountNumber);
		nscore3HomePage.runAutoshipOrder();
		s_assert.assertTrue(nscore3HomePage.getFailedTemplateMessage().contains(errorMsg),"Template already processed this month. message not visible");
		s_assert.assertAll();
	}

//QTest-238  Admin-Create new user
	@Test 
	public void testAdminCreateNewUser_238(){
		nscore3LoginPage = new NSCore3LoginPage(driver);
		nscore3HomePage = new NSCore3HomePage(driver);
		int randomNum = CommonUtils.getRandomNum(10000, 1000000);
		String UserName=TestConstantsRFL.FIRST_NAME+randomNum;
		String FirstName = TestConstantsRFL.FIRST_NAME;
		String LastName = TestConstantsRFL.LAST_NAME+randomNum;
		String Email = FirstName+randomNum+"@rodanandfields.com";
		String password = propertyFile.getProperty("storeFrontPassword");
		nscore3HomePage.clickAdminLink();
		nscore3HomePage.clickCreateNewUser();
		nscore3HomePage.enterNewUserDetails(UserName, FirstName, LastName, Email, password);
		nscore3HomePage.SelectRole();
		nscore3HomePage.selectSiteAccess();
		nscore3HomePage.enterLoginMessage();
		nscore3HomePage.clickSaveChangesLinkOnCreateNewUser();
		s_assert.assertTrue(nscore3HomePage.verifyAdminTabIsDisplayed(),"Administration Tab is not displayed");
		nscore3HomePage.searchNewUser(UserName);
		s_assert.assertTrue(nscore3HomePage.verifyUsernameOfNewlyCreatedUser().contains(UserName),"User Name doesnot match with the user created");
		s_assert.assertTrue(nscore3HomePage.verifyEmailIdOfNewlyCreatedUser().contains(Email),"Email ID doesnot match with the user created");
		s_assert.assertAll();
	}

	//QTest-223  Orders - User is able to search for orders by consultant name
	@Test 
	public void testOrdersUserIsAbleToSearchForOrdersByConsultantName_223(){
		String orderID = "Order ID";
		String orderNumber = "Order Number";
		String customer = "Customer";
		String subTotal = "Subtotal";
		...
		logger.info("First name from DB is "+firstName);
		logger.info("Last name from DB is "+lastName);
		consultantName = firstName.trim() +" "+ lastName.trim();
		nscore3HomePage.clickOrdersLink();
		nscore3HomePage.clickSearchByDD();
		nscore3HomePage.selectValueFromSearchByDD("Consultant Name");
		nscore3HomePage.enterSearchValue(consultantName);
		nscore3HomePage.clickSearch();

		s_assert.assertTrue(nscore3HomePage.isColumnNamePresent(consultant), "customer column is not present");

		int consultantColumnNumber = nscore3HomePage.getColumnNumberHavingExpectedColumnName(consultant);
		int rowNumber = nscore3HomePage.getRowNumberWithExpectedName(consultantName, consultantColumnNumber);
		s_assert.assertTrue(nscore3HomePage.isExpectedValuePresentInColumn(consultantName, consultantColumnNumber), consultantName+" NOT found in search result");
		//s_assert.assertFalse(nscore3HomePage.verifyConsultantname().contains(consultantName), "values are equal");
	...
	//QTest ID TC-1899 Run CRP
		@Test 
		public void testRunCRP_1899(){
			RFL_DB = driver.getDBNameRFL();
			String customerName = null;
			String accountNumber = null;
			String firstName = null;
	...
	//QTest ID TC-1901 Run PC
		@Test 
		public void testRunPC_1901(){
			RFL_DB = driver.getDBNameRFL();
			String customerName = null;
			String accountNumber = null;
			String firstName = null;
