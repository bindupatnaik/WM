...
@Listeners({com.rf.core.listeners.TestListner.class,org.uncommons.reportng.HTMLReporter.class})
public class OrderValidationTest extends RFWebsiteBaseTest{
	...
	/**
	 * Test Summary:- login with consultant, goto orders page and click on Actions->Report Problems
	 * Assertions:
	 * Order Page URL
	 * 
	 * @throws SQLException
	...
	public void testOrdersReportProblems_1980() throws SQLException, InterruptedException{
		...
		storeFrontOrdersPage.setOrderNumberFromOrderHistory();
		...
		s_assert.assertTrue(storeFrontReportOrderComplaintPage.verifyCountOfDropDownOptionsOnReportPage(6),"DropDown Options are not present as expected");
	...
	public void testCheckOrdersDetailsFromActionsTab_1982() throws InterruptedException{
		...
		if(driver.getCountry().equalsIgnoreCase("au"))
			subTotalDB = df.format((Number)getValueFromQueryResult(getOtherDetailValuesList, "DisplaySubTotal"));
		else
		...
		if(driver.getCountry().equalsIgnoreCase("au"))
			shippingDB = df.format((Number)getValueFromQueryResult(shippingCostAndHandlingCostList, "DisplayShippingCost"));
		else
	...
	public void testOrderPageUIAndEditCartForPC_1979() throws InterruptedException{
		...
		tax = storeFrontUpdateCartPage.getTotalTaxAmountFromCart();
	...
	@Test
	public void testAdhocOrdersForMultiplsLineItem_2273_1880() throws InterruptedException  {
		RFO_DB = driver.getDBNameRFO(); 
		List<Map<String, Object>> randomConsultantList =  null;
		String consultantEmailID = null;
		String accountID = null;
	...
	//QTest ID TC-530 Adhoc Orders for Consultant and PC and RC --> Single line Item
	//Hybris Project-2272:Adhoc Orders from Consultant and PC and RC --> Single line Item
	@Test
	public void testAdhocOrdersFromConsultantAndPc_530q() throws InterruptedException{
		RFO_DB = driver.getDBNameRFO();
		List<Map<String, Object>> randomConsultantList =  null;
		String consultantEmailID = null;
		String accountID = null;
	...
	@Test(enabled=false)//covered in TC 3993 in homePage
	public void checkDetailsOfCRpAutoshipForInactiveConsultant_4304() throws InterruptedException{
		...
		s_assert.assertTrue(shippingMethodFromUI.equalsIgnoreCase(shippingMethodDB),"CRP autoship template shipping method on RFO is "+shippingMethodDB+" and on UI is "+shippingMethodFromUI);
		s_assert.assertAll();
	}	
	...
	public void testOrdersDetailsOfConsultant_HP2_4287() throws SQLException, InterruptedException{
		...
		if(isShippingMethodPresent==true){
			assertTrue(storeFrontOrdersPage.getShippingMethodFromAutoshipTemplate().equalsIgnoreCase(shippingMethodDB),"Adhoc Order template shipping method on RFO is "+shippingMethodDB+" and on UI is "+storeFrontOrdersPage.getShippingMethodFromAutoshipTemplate());
		}
		s_assert.assertAll();
	}
	...
	public void testOrderDetailsForAdhocOrdersForRC_4293() throws InterruptedException{
		...
		if(shippingMethodDB.contains("(")){
			String[] name = shippingMethodDB.split("\\(");
			shippingMethodName = name[0]+" "+"("+name[1];
		}else{
			shippingMethodName= shippingMethodDB;
		}
		//Assert Subtotal with RFO
	...
	public void testReturnOrderHistory_1983() throws InterruptedException{
		...
		if(driver.getCountry().equalsIgnoreCase("au"))
			subTotalDB = df.format((Number)getValueFromQueryResult(getOrderDetailsList, "DisplaySubTotal"));
		else
	...
	//QTest ID TC-543 Place an Adhoc Order For Consultant
	// Hybris Phase 2-1878 :: Version : 1 :: Create Adhoc Order For The Consultant Customer
	@Test
	public void testCreateAdhocOrderConsultant_543q() throws InterruptedException{
		int randomNum = CommonUtils.getRandomNum(10000, 1000000);
		String lastName = "lN";
		String firstName=TestConstants.FIRST_NAME+randomNum;
		String consultantEmailID = QueryUtils.getRandomActiveConsultantFromDB(RFO_DB, DBQueries_RFO.GET_RANDOM_ACTIVE_CONSULTANT_WITH_ORDERS_AND_AUTOSHIPS_RFO,countryId);
	...
	//QTest ID TC-2778 Place an Adhoc Order For RC
	// Hybris Phase 2-1879 :: Version : 1 :: Create Adhoc Order For The Retail Customer
	@Test
	public void testCreateAdhocOrderRC_2778q() throws InterruptedException {
		int randomNum = CommonUtils.getRandomNum(10000, 1000000);
		RFO_DB = driver.getDBNameRFO();
		List<Map<String, Object>> randomRCList =  null;
		String rcUserEmailID =null;
	...
	//QTest ID TC-2777 Place an Adhoc Order For PC
	//QTest ID TC-2985 Place an Adhoc Order For PC
	// Hybris Phase 2-1877 :: Version : 1 :: Create Adhoc Order For The Preferred Customer 
	@Test
	public void testCreateAdhocOrderPC_2777q_2985q() throws InterruptedException{
		int randomNum = CommonUtils.getRandomNum(10000, 1000000);
		RFO_DB = driver.getDBNameRFO();
		List<Map<String, Object>> randomPCUserList =  null;
		String pcUserEmailID = null;
	...
	//QTest ID TC-531 Place Adhoc Order as RC with different sponsor
	//Hybris Project-3779:Place Adhoc Order as RC with different sponsor
	@Test(enabled=false)//Test needs update
	public void testPlaceAdhocOrderAsRCWithDifferentSponsor_531q() throws InterruptedException{
		RFO_DB = driver.getDBNameRFO();
		List<Map<String, Object>> randomRCList =  null;
	...
	@Test
	public void testVariantProductFromCategoryForConsultant() throws InterruptedException{
		if(driver.getCountry().equals("ca")){
			int randomNum = CommonUtils.getRandomNum(10000, 1000000);
			String productFirstName = TestConstants.VARIANT_PRODUCT_FIRST_NAME;
			String productLastName = TestConstants.VARIANT_PRODUCT_LAST_NAME;
			String shade = "Medium";    
			String firstName=TestConstants.FIRST_NAME+randomNum;
			String lastName = "lN";
			List<Map<String, Object>> randomConsultantList =  null;
			...
			storeFrontHomePage.hoverOnShopLinkAndClickCategoryLink("Enhancements");
			s_assert.assertTrue(storeFrontHomePage.isAlertPresentForVariantProductsAdHoc(productFirstName+productLastName),"Alert not displayed while adding product to Ad-hoc cart without selecting a Variant");
			storeFrontHomePage.selectVariantAndAddProductToAdHocCart(productFirstName+productLastName, shade);
			s_assert.assertTrue(storeFrontHomePage.isVariantProductAvailableInCart(productFirstName, productLastName, shade),"Variant Product is not available on Ad-hoc cart page");
			storeFrontUpdateCartPage.clickOnCheckoutButton();
			storeFrontUpdateCartPage.clickOnShippingAddressNextStepBtn();
			storeFrontUpdateCartPage.clickOnDefaultBillingProfileEdit();
			storeFrontUpdateCartPage.enterBillingDetailsAndSave(cardNumber, firstName+" "+lastName, cardExpMonth, cardExpYear, CVV);
			storeFrontUpdateCartPage.clickOnBillingNextStepBtn();
			storeFrontUpdateCartPage.clickPlaceOrderBtn();
			s_assert.assertTrue(storeFrontUpdateCartPage.verifyOrderPlacedConfirmationMessage(), "Order has been not placed successfully");
			storeFrontUpdateCartPage.clickOnRodanAndFieldsLogo();
			...
			storeFrontHomePage.removeProductFromCart(productFirstName, productLastName, shade);
			storeFrontHomePage.clickOnRodanAndFieldsLogo();
			storeFrontHomePage.hoverOnShopLinkAndClickCategoryLink("Enhancements");
			s_assert.assertTrue(storeFrontHomePage.isAlertPresentForVariantProductInAutoship(productFirstName+productLastName),"Alert not displayed while adding product to autoship cart without selecting a Variant");
			storeFrontHomePage.selectVariantAndAddProductToAutoshipCart(productFirstName+productLastName, shade);
			s_assert.assertTrue(storeFrontHomePage.isVariantProductAvailableInCart(productFirstName, productLastName, shade),"Variant Product is not available on autoship cart page");
			storeFrontHomePage.clickOnRodanAndFieldsLogo();
			s_assert.assertAll();
		}
	}
